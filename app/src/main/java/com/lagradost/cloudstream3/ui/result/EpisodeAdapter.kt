        ...
        when (val binding = holder.view) {
            is ResultEpisodeLargeBinding -> {
                ...
                binding.apply {
                    ...
                    val name =
                        if (item.name == null) "${episodeText.context.getString(R.string.episode)} ${item.episode}" else item.name
                    ...
                    episodeIndex.context?.getShortSeasonText(item.episode, item.season)?.let { text->
                        episodeIndex.text = text
                        episodeIndex.isVisible = true
                    }
